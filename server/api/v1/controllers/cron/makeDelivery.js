...
const createDelhiveryOrder = async (shippingObject) => {
  try {
    ...
    const response = await axios.post(`${DELHIVERY_API_URL}/api/cmu/create.json`, payload, {
      headers: {
...
const processTransactions = async () => {
  try {
    const transactions = await findTransactions({ orderCreated: false,status:"PENDING" });

    if (transactions.length === 0) return;
let wareHouse =await findWareHouse({})
if(wareHouse){
  for (const trx of transactions) {
    if (trx.paymentType === "COD" || (trx.paymentType === "Pre-Paid" && trx.paymentStatus === "COMPLETED")) {
      ...
      if (orderCreated) {
        await updateTransaction({ _id: trx._id }, { wayBill:wayBill, orderCreated: true });
        for(let i=0;i<trx.products.length;i++){
          let product = await findProduct({ _id: trx.products[i].productId });
          if(product){
            let quantity = product.stock - trx.products[i].quantity
            await updateProduct({ _id: product._id }, { quantity: quantity>0?quantity:0 });
          }
        }
        console.log(`✅ Order created successfully for Transaction ID: ${trx._id}`);
      }
    }
  }
}
